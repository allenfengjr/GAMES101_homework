...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    A=v[0];B=v[1];C=v[2];
    Eigen::Vector4f* abc=new Eigen::Vector4f [3];
    abc[0]={A.x(),A.y(),A.z(),1.0f};
    abc[1]={B.x(),B.y(),B.z(),1.0f};
    abc[2]={C.x(),C.y(),C.z(),1.0f};
    float X_min,X_max,Y_min,Y_max;
    X_max=std::max(A.x(),B.x());
    X_max=std::max(X_max,C.x());
    X_min=std::min(A.x(),B.x());
    X_min=std::min(X_min,C.x());
    Y_max=std::max(A.y(),B.y());
    Y_max=std::max(Y_max,C.y());
    Y_min=std::min(A.y(),B.y());
    Y_min=std::min(Y_min,C.y());
    // TODO : Find out the bounding box of current triangle.
    // iterate through the pixel and find if the current pixel is inside the triangle
    for(int x=X_min;x<=X_max;x++){
        for(int y=Y_min;y<=Y_max;y++){
            if(insideTriangle(x+0.5,y+0.5,abc)){
